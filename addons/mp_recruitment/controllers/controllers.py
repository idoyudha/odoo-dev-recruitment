...
class MPRecruitment(http.Controller):
    _jobs_per_page = 4

    @http.route('/mpjobs', auth='public')
    def mpjobs(self):
        jobs = request.env["hr.job"].search([])
        _logger.debug(jobs)

        data = random.sample([job for job in jobs], k=min(self._jobs_per_page, len(jobs)))
        _logger.debug(data)

        return http.request.render('mp_recruitment.jobs', {
            'jobs': data,
        })
    ...
    def jobs(self, country_id=None, department_id=None, office_id=None, contract_type_id=None,
        ...
        jobs = random.sample([job for job in jobs], k=min(self._jobs_per_page, len(jobs)))
